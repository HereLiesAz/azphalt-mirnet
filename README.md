...
**MIRNet (image restoration)** — Multi-scale image restoration for low-light enhancement and denoising.
...
- low-light / night footage recovery
- denoising
- detail & contrast enhancement

## Roles (host-neutral routing)

This plugin contributes the role(s): `lowlight`. A host routes the model by role — it carries no
`targetApps`, so it is not tied to any single application.

**Example host — [Guillotine](https://github.com/HereLiesAz/Guillotine):** Desktop `effect_lowlight` — `apply_image_effect effect=lowlight`.

## Model file(s)

- **`mirnet.onnx`** (role `lowlight`) — [upstream](https://huggingface.co/onnx-community/mirnet/resolve/main/mirnet.onnx)

Model license: **MIT (MIRNet)**. This plugin's manifest/packaging is `MIT`.
...
```sh
npm install && npm run build     # packs com.hereliesaz.azphalt.mirnet-1.0.0.azp
